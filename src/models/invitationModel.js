...
const INVITATION_COLLECTION_SCHEMA = Joi.object({
  ...
  inviteeId: Joi.string().required().pattern(OBJECT_ID_RULE).message(OBJECT_ID_RULE_MESSAGE), // người nhận lời mời
...
const findOneByEmail = async (emailValue) => {
  try {
    const result = await GET_DB().collection(INVITATION_COLLECTION_NAME).findOne({ email: emailValue })
    return result
  } catch (error) {throw new Error(error)}
}
